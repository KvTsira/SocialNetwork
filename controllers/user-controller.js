...
const userController = {
  ...
  async getUserByID({ params }, res) {
    try {
      ...
      if (!dbUser)
        return res
          .status(404)
          .json({ message: "Can not find user with the given id!" });
      res.json(dbUser);
    } catch (err) {
      res.status(500).json(err);
    }
  ...
  async updateUserByID({ params, body }, res) {
    try {
      const dbUser = await User.findOneAndUpdate({ _id: params.id }, body, {
        new: true,
        runValidator: true,
      });
      if (!dbUser)
  ...
  async removeUserByID({ params }, res) {
    try {
      const dbUser = await User.findOneAndDelete({ _id: params.id });
      if (!dbUser)
        return res
          .status(404)
          .json({ message: "Can not find a user with the given id!" });
      const dbThought = await Thought.deleteMany({
        _id: { $in: dbUser.thoughts },
      });
      res.json({
        dbUser,
        dbThought,
